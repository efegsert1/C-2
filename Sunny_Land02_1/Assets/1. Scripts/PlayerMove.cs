...
public class PlayerMove : MonoBehaviour
{
    ...
    //스프라이트 렌더러 변수 선언
    //public (접근 제한자)
    //다른 클래스에서도 현재 클래스에 접근이 가능하다.
    //만약에 public 붙어있지 않다면 현재 클래스에서만 사용이 가능하다.
    ...
    {
        //좌우 키보드 누른 값 저장
        //GetAxisRaw : 키보드, 조이스틱,마우스의 입력값을 -1 0 1로 즉각 반환하는 함수입니다.
        ...
        //Impulse : 질량에 영향을 받으며 한번에 순간적으로 힘을 가함(점프, 총알 공격)
        ...
        if (xinput != 0)
        {
            //Horizontal이 왼쪽이라면 x를 실행시키기
            sr.flipX = Input.GetAxisRaw("Horizontal") == -1;

            //걷는 애니메이션을 실행
            //SetBool :파라미터를 Bool로 만들었기 때문에 
